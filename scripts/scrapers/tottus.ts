...
export async function scrapeTottus(page: Page, url: string) {
  ...
  const priceText = await page
    .locator(".price, .product-price, [data-testid='price']")
    .first()
    .innerText()
    .catch(() => "");

  const price = normalizePrice(priceText);
  if (!price) throw new Error("No se encontró precio en Tottus");
  return price;
}
