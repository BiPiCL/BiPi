...
export async function scrapeUnimarc(page: Page, url: string) {
  ...
  const priceText = await page
    .locator(".product-price, .prices__price, [data-testid='price']")
    .first()
    .innerText()
    .catch(() => "");

  const price = normalizePrice(priceText);
  if (!price) throw new Error("No se encontró precio en Unimarc");
  return price;
}
